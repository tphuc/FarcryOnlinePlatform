...
def post_data(param_match, param_frags, username, id_user, token, param_setting):
    ...
    url_settings = 'http://127.0.0.1:8000/api/user/{}/settings'.format(id_user)
    if username != username_master:
        url_match = 'http://127.0.0.1:8000/api/matches.create'
        r = requests.post(url=url_match, data=param_match)
        id = r.json()['match']['id']
        url_matchfrags = 'http://127.0.0.1:8000/api/frags.create'
